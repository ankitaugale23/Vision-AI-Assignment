...
import { ensureOutputDir } from './utils/comparision.js';
...
async function createHighlightedImage(imagePath, differences, processedDims) {
  if (!differences || !differences.differences || differences.differences.length === 0) {
    console.log("No differences detected to highlight");
    return null;
  }
  ...
  try {
    ...
    const scaleX = (image.width - 1) / (processedDims.width - 1);
    const scaleY = (image.height - 1) / (processedDims.height - 1);
    ...
    differences.differences.forEach((diff, index) => {
      if (diff.highlight_area) {
        ...
        const scaledX2 = Math.min(image.width, (x2 * scaleX) + xOffset);
        const scaledY2 = Math.min(image.height, (y2 * scaleY) + yOffset);
